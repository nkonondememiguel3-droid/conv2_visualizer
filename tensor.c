...
static void compute_strides( _tensor_t *tensor )
{
  ...
  for ( int i = tensor->dimension - 2; i >= 0; i-- ) tensor->strides[i] = tensor->strides[i + 1] * tensor->shape[i + 1];
}
...
_tensor_t *tensor_ones( _ds_arena_t_ *arena, int dimension, const int shape[] )
{
  ...
  for ( int i = 0; i < tensor->size; i++ ) tensor->data[i] = 1.0f;
