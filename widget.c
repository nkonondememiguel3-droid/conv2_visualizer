...
{
  if ( nk_begin( ctx, "Template", nk_rect( 50, 50, 220, 120 ), NK_WINDOW_BORDER | NK_WINDOW_TITLE | NK_WINDOW_CLOSABLE | NK_WINDOW_MOVABLE ) )
  {
...
bool example( struct nk_context *ctx )
{
  ...
  if ( nk_begin( ctx, "example", nk_rect( 400, 50, 220, 220 ), NK_WINDOW_BORDER | NK_WINDOW_MOVABLE | NK_WINDOW_CLOSABLE ) )
  {
    nk_layout_row_static( ctx, 30, 80, 1 );
    if ( nk_button_label( ctx, "button" ) ) printf( "button clicked\n" );
...
{
  if ( !tensor_data || width < 0 || height < 0 ) return;
